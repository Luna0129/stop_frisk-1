...
moran.loc.rook <-localmoran(racemerged2@data$rate.clothing,lws,p.adjust.method="fdr", zero.policy = T) 
...
moran.loc.queen <-localmoran(racemerged2@data$rate.clothing,lws,p.adjust.method="fdr", zero.policy = T) 
shades <- shading(c(0.05),cols=c(2,8))
...
moran.loc.rook <-localmoran(racemerged2@data$rate.furtive,lws,p.adjust.method="fdr", zero.policy = T) 
shades <- shading(c(0.05),cols=c(2,8))
...
moran.loc.queen <-localmoran(racemerged2@data$rate.furtive,lws,p.adjust.method="fdr", zero.policy = T) 
shades <- shading(c(0.05),cols=c(2,8))
